...
// .\..\..\.build\win64_vs2022\bin\shadercDebug.exe -f fs_50_mesh.sc -o fs_50_mesh.bin --type f --platform windows --debug --profile s_5_0 -i .\..\..\src
...
float3 reflected = _lightDir - 2.0*ndotl*_normal;
...
void main( float4 gl_FragCoord : SV_POSITION , float4 v_color0 : COLOR0 , float3 v_normal : NORMAL , float3 v_pos : TEXCOORD1 , float3 v_view : TEXCOORD2 , out float4 bgfx_FragData0 : SV_TARGET0 )
{
...
float2 bln = blinn(lightDir, normal, view);
...
float3 color = float3(sin(index*8.0)*0.4 + 0.6
, sin(index*4.0)*0.4 + 0.6
, sin(index*2.0)*0.4 + 0.6
) * v_color0.xyz;
bgfx_FragData0.xyz = pow(float3(0.07, 0.06, 0.08) + color*lc.y + fres*pow(lc.z, 128.0), vec3_splat(1.0/2.2) );
